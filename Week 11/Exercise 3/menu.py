# Contans the logic for the menu
import os
from Student import Student

def pause():
    input("\nPress any key to continue...")

def clear_screen():
    # For Windows
    if os.name == 'nt':
        os.system('cls')
    # For Linux or macOS
    else:
        os.system('clear')

def display_message(tag, message, do_pause=False):
    print(f"""
                
            {tag}
            {message}
                
            """)
    if do_pause:
        pause()


def request_grade(message):
    grade = 0
    while True:
        try:
            grade = float(input(message))
            if not (0 <= grade <= 100):
                raise ValueError
            return grade
        except ValueError:
            display_message('WARNING', 'The grade must be a value from [0,100] both inclusive')


def display_main_menu():
    try_again = True
    while try_again:
        menu = """
                            Menu
            1. Register a Student.
            2. List all students.
            3. List The Top 3 Best Avarge Students
            4. Show the avaragre of all students avarage
            5. Export Data to Csv
            6. Import Data from previous exported Csv
            7. Exit
        """
        clear_screen()
        print(menu)
        try:
            option = int(input("Select an option: "))
            if not (0 < option < 8):
                raise ValueError
            try_again = False
        except ValueError as valueError:
            display_message('WARNING','the value must be an option from [1,7] both inclusive')
            pause()
    return option

def register_student():
    menu = """
                New Student Registration
    """
    clear_screen()
    print(menu)

    full_name = input("Full Name: ")
    section = input("Section: ")
    grade_in_spanish = request_grade("Grade in Spanish: ")
    grade_in_english = request_grade("Grade in English: ")
    grade_in_science = request_grade("Grade in Science: ")
    grade_in_social_studies= request_grade("Grade in Social Studies: ")

    student = Student(
        full_name= full_name,
        section= section,
        grade_in_english= grade_in_english,
        grade_in_spanish= grade_in_spanish,
        grade_in_science= grade_in_science,
        grade_in_social_studies= grade_in_social_studies
    )

    pause()

    return student

def display_all_students(student_list):
    menu = """
                List of Students
    """
    clear_screen()
    print(menu)

    print(f"{'Full Name':<20} {'Section':<8} {'Spanish':<8} {'English':<8} {'Science':<8} {'Social Studies':<15}")
    print("-" * 90)

    for student in student_list:
        print(f"{student.full_name:<20} {student.section:<8} {student.grade_in_spanish:<8} {student.grade_in_english:<8} {student.grade_in_science:<8} {student.grade_in_social_studies:<15}")

    pause()


def display_top_3_students(student_list):
    menu = """
            TOP 3 BEST STUDENTS AVERAGE
    """
    clear_screen()
    print(menu)

    print(f"{'Full Name':<20} {'Section':<8} {'Average':<8}")
    print("-" * 40)

    for index, student in enumerate(student_list):
        pos = index + 1
        print(f"{pos} | {student.full_name:<20} {student.section:<8} {student.get_average():<6.1f}")

    pause()

def display_average_of_all_average(Average):
    menu = """
            AVARGE FROM ALL STUDENTS AVERAGE
    """
    clear_screen()
    print(menu)

    print(f"{'Average':<8}")
    print("-" * 20)
    print(Average)

    pause()